...
namespace Pet
{
    public class petTest
    {
        ...
        [TestCaseSource("getTestData", new object[] { }), Order(6)]
        ...
        {
            // Configura um objeto PetModel com os dados do teste
            PetModel petModel = new PetModel();
            petModel.id = petId;
            ...
            String[] tagsIdsList = tagsIds.Split(";");   // Lê e divide as IDs das tags
            String[] tagsNameList = tagsName.Split(";"); // Lê e divide os nomes das tags
            List<Tag> tagList = new List<Tag>(); // Lista para armazenar as tags após o loop
            ...
            for (int i = 0; i < tagsIdsList.Length; i++)
            {
                ...
                tagList.Add(tag); // Adiciona cada tag à lista
            }

            petModel.tags = tagList.ToArray(); // Converte a lista de tags em um array
            ...
            // Cria um cliente REST com o endereço da API
            var client = new RestClient(BASE_URL);

            // Cria uma solicitação HTTP do tipo POST para a rota "pet"
            var request = new RestRequest("pet", Method.Post);
            ...
            // Valida o código de status da resposta é igual a 200 (sucesso)
            Assert.That((int)response.StatusCode, Is.EqualTo(200));
